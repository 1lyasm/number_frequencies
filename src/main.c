...
void fill_s(int *arr, int n, int *val_stack, int *freq_stack, int *sp, int i) {
  if (i >= n) {
    return;
  }
  if (i == 0) {
...
void fill_m(struct Matrix *m, int *val_stack, int *freq_stack, int *sp, int i) {
  if (i > (*sp)) {
    return;
  }
